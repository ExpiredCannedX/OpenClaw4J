...
- 当前工具调用安全 P0 已归档，包括统一服务端策略层、显式确认态、filesystem 写操作参数级校验、结构化审计日志和不可信内容防绕过边界。

**Roadmap 目标**
- 继续补齐健康检查、更多 sink、事件持久化与更系统化的运维面。
- 在已归档安全 P0 的基础上，继续推进更细粒度的策略、预览能力和长期治理能力。
- 在不破坏本地优先原则的前提下，逐步扩展更成熟的恢复与治理机制。

## 工具调用安全治理
**当前已归档能力**
- 当前所有本地工具与 MCP 工具都会在真实执行前经过统一的服务端策略层，并基于服务端安全画像、标准化参数、运行时上下文和确认状态判定为 `allowed`、`denied` 或 `confirmation_required`。
- 当前高风险工具请求会持久化待确认记录，并把内部会话、内部用户、工具名、参数指纹、风险摘要、状态和过期时间绑定到服务端状态。
- 当前 `filesystem` 写能力已具备路径规范化、workspace-root 约束、符号链接逃逸检查、敏感路径 denylist，以及递归/批量修改边界校验。
- 当前 unsafe filesystem MCP 写请求会在 transport 之前被拒绝，不会把风险请求发给远端 MCP session。
- 当前已把工具输出、网页内容、workspace 文件内容和记忆内容视为不可信输入，不允许它们创建确认态、覆盖 denylist 或绕过服务端拒绝。
- 当前高风险工具调用已具备结构化审计日志，覆盖请求、判定、确认与最终执行结果。
